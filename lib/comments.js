...
Meteor.methods({
  createComment: function(itemId, teamId, channelId, comment) {
    ...
    const allMembers = team.members.concat({ _id: team.owner }).map(function(teamMember) {
    ...
    if (Meteor.isServer) {
      if (team.slackWebhookUrl) {
        Slack.notify(userEmail(userId) + ' just commented on ' +
                     '<' + Meteor.absoluteUrl() + team.slug + '/' + channel.slug +
                       '/' + item._id + '|' + item.title + '> ' +
                     'and said:\n' +
                     '> ' + comment, team.slackWebhookUrl);
      }
    }
  },
