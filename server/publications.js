...
Meteor.publishComposite('teams', {
  ...
  children: [
    ...
    { find: function(team) {
      return Feeds.find(
        { _id: { $in: _(team.channels).pluck('feeds').flatten().value() }},
        { fields: {} }
      );
    }}
...
Meteor.publishComposite('item', function(options) {
  ...
  return {
    find: function() {
      ...
      const channel = _(teamForItem[0].channels).find({ slug: options.channelSlug });
      ...
      return Items.find({ _id: new Meteor.Collection.ObjectID(options.itemId) });
    },
    children: [
      { find: function(item) {
        return TeamItems.find({
